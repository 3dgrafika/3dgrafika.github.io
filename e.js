...
function displayResult(filteredData) {
  ...
  const paginationContainer = document.getElementById('pagination'); // Dodane

  let html = '';
  let paginationHtml = ''; // Dodane
  ...
  paginationHtml += `<ul class="uk-pagination" uk-margin>`;

  // Strzałka w lewo (poprzednia strona)
  if (currentPage > 1) {
    paginationHtml += `<li><a href="" onclick="changePage(${currentPage - 1})"><span uk-pagination-previous></span></a></li>`;
  } else {
    paginationHtml += `<li class="uk-hidden"><span uk-pagination-previous></span></li>`;
  }
  ...
  for (let i = 1; i <= totalPages; i++) {
    if (i === 1 || i === totalPages || (i >= currentPage - 2 && i <= currentPage + 2)) {
      ...
      paginationHtml += `<li class="uk-disabled"><span>…</span></li>`;
    }
  }

  // Strzałka w prawo (następna strona)
  if (currentPage < totalPages) {
    paginationHtml += `<li><a href="" onclick="changePage(${currentPage + 1})"><span uk-pagination-next></span></a></li>`;
  } else {
    paginationHtml += `<li class="uk-hidden"><span uk-pagination-next></span></li>`;
  }

  paginationHtml += `</ul>`;
  
  // Dodanie paginacji do kontenera
